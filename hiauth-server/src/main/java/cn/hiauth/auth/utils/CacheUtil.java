...
public final class CacheUtil {
    ...
    public CacheUtil(RedisTemplate<String, Object> redisTemplate, String keyPrefix) {
    ...
    public Set<String> keys(String key) {
        ...
        Set<String> kSet = redisTemplate.keys(getFullKey(key));
        if (kSet == null) {
            return null;
        }
        ...
        while (it.hasNext()) {
            ...
            if (keyPrefix.length() >= 0) {
                keys.add(k.substring(keyPrefix.length() + 1));
            } else {
                keys.add(k);
            }
        }
    ...
    public void del(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                redisTemplate.delete(getFullKey((List<String>) CollectionUtils.arrayToList(key)));
            }
        }
    }
    ...
    public boolean set(String key, Object value) {
        try {
            redisTemplate.opsForValue().set(getFullKey(key), value, EXPIRE, TimeUnit.SECONDS);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public Map<String, Object> hmget(String key) {
        ...
        return operations.entries(getFullKey(key));
    }
    ...
    public double hincr(String key, String item, double by) {
        return redisTemplate.opsForHash().increment(getFullKey(key), item, by);
    }
    ...
    public long sSet(String key, Object... values) {
        try {
            return redisTemplate.opsForSet().add(getFullKey(key), values);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    ...
    public long setRemove(String key, Object... values) {
        try {
            Long count = redisTemplate.opsForSet().remove(getFullKey(key), values);
            return count;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public long lRemove(String key, long count, Object value) {
        try {
            Long remove = redisTemplate.opsForList().remove(getFullKey(key), count, value);
            return remove;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;

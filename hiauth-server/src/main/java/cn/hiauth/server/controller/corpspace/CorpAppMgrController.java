...
import cn.hiauth.server.api.dto.corpApp.CorpAppCreateDto;
import cn.hiauth.server.api.dto.corpApp.CorpAppLimitDto;
import cn.hiauth.server.api.dto.corpApp.CorpAppPageDto;
import cn.hiauth.server.api.dto.corpApp.CorpAppUpdateDto;
...
@ResourceApi
@RestController
@RequestMapping("/api/corpSpace/corpAppMgr")
public class CorpAppMgrController {
    ...
    @PostMapping("/page")
    public R<PageVO<CorpAppVo>> page(@RequestBody @Valid CorpAppPageDto dto) {
    ...
    @PostMapping("/add")
    public R<Boolean> add(@RequestBody @Valid CorpAppCreateDto dto) {
        Long cid = SessionContextHolder.getPrincipal().getCid();
        Assert.notNull(cid, SysCode.biz(1), "未登录租户空间");
        dto.setCorpId(cid);
    ...
    @PostMapping("/update")
    public R<Oauth2RegisteredClient> update(@RequestBody @Valid CorpAppUpdateDto body) {
        Long cid = SessionContextHolder.getPrincipal().getCid();
        Assert.notNull(cid, SysCode.biz(1), "未登录租户空间");
    ...
    public R<List<App>> limitNotHaveApp(@RequestBody @Valid CorpAppLimitDto dto) {
        Long cid = SessionContextHolder.getPrincipal().getCid();
        Assert.notNull(cid, SysCode.biz(1), "未登录租户空间");
        dto.setCid(cid);
    ...
    public R<List<App>> limitHaveApp(@RequestBody @Valid CorpAppLimitDto dto) {
        Long cid = SessionContextHolder.getPrincipal().getCid();
        Assert.notNull(cid, SysCode.biz(1), "未登录租户空间");
        dto.setCid(cid);
